...
public class TokenSolver {
    ...
    public RSAKeyProvider keyProvider = new RSAKeyProvider() {
        ...
        public RSAPublicKey getPublicKeyById(String s) {
            if (key == null){
                key = RSAKeysReader.readRsaPub(pubFile);
            }
            return key;
        }
...
//        }
        ...
        jwtUser.setOpenId(claims.get("openId").asString());
        jwtUser.setAvatarUrl(claims.get("avatarUrl").asString());
        jwtUser.setNickName(claims.get("nickName").asString());
